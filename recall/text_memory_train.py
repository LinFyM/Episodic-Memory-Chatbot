...
from server.memory.utils import inject_memory_embedding_to_inputs_embeds

def enhanced_collate_fn(batch):
    """简化版collate函数 - 支持新的记忆解码训练格式"""
...
class EnhancedTextMemoryDataset(Dataset):
    ...
    def __init__(
        ...
        self.activation_prompts = activation_prompts or ["让我回忆一下："]
        self.end_prompts = end_prompts or ["——以上是我记得的内容。"]
        if not isinstance(self.activation_prompts, list) or len(self.activation_prompts) == 0:
            self.activation_prompts = ["让我回忆一下："]
        if not isinstance(self.end_prompts, list) or len(self.end_prompts) == 0:
            self.end_prompts = ["——以上是我记得的内容。"]
    ...
    def _truncate_sft_at_thinking(self, sft_data: dict) -> str:
        ...
        Args:
            sft_data: 包含完整文本和思考部分起止位置的字典
                {"full_text": str, "thinking_start": int, "thinking_end": int}
        ...
        if not thinking_content.strip():
            # 如果思考部分为空，返回思考开始之前的所有内容
            return full_text[:thinking_start]
        ...
        if len(thinking_tokens) <= 1:
            # 如果思考部分太短，返回思考开始之前的所有内容
            return full_text[:thinking_start]
        
        # 随机选择一个截断位置（至少保留1个token，最多保留到倒数第二个token）
        ...
        for i, token_id in enumerate(thinking_tokens):
            ...
            try:
                ...
                if any(punct in token_text for punct in ['。', '.', '！', '!', '？', '?', '；', ';']):
                    sentence_end_tokens.append(i + 1)  # 在标点后面截断
            except:
                pass
        
        if sentence_end_tokens:
            # 如果有句号等，随机选择一个句号后面的位置
        ...
        # 构造截断后的完整文本：从开始到思考部分开始 + 思考部分截断后的内容 + 思考开始标签
        truncated_text = (
            full_text[:thinking_start + len(start_tag)] +  # 从开始到思考开始标签
            truncated_thinking_text  # 思考部分截断后的内容
        )
        
        return truncated_text.strip()
    
    def _get_memory_decode_sample(self, text_idx):
        """构造记忆解码训练样本
        ...
        if len(self.sft_full_texts) > 0:
            ...
            # 在思考部分内部随机截断（优先在句号后面），返回从开始到截断点的全部内容
            context_text = self._truncate_sft_at_thinking(sft_data)
        if context_text:
                # 将上下文编码为token（不限制长度，后面会处理总长度）
                context_tokens = actual_tokenizer(context_text, add_special_tokens=False)['input_ids']
        elif len(self.texts) > 1:
            # 如果没有SFT数据，回退到从记忆条目中选择（排除当前索引）
...
class EnhancedTextMemoryModel(nn.Module):
    ...
    def forward(self, input_ids, embeddings_to_insert, embedding_positions, attention_mask, labels=None, memory_pad_token_id=None):
        """
        前向传播
        
        Args:
            input_ids: [batch_size, seq_len] token序列
            embeddings_to_insert: [batch_size, embed_dim] 要插入的表征向量
            embedding_positions: [batch_size] 表征向量插入位置
...
# 设备变量已在前面设置
    ...
    def _freeze_embeddings_except_special_tokens(self):
        ...
        # 然后只解冻可训练的特殊token的embedding参数（不包括<|memory_pad|>）
        ...
        # 验证是否确实可训练
        is_trainable = all(embedding_layer.weight[token_id].requires_grad for token_id in self.trainable_special_token_ids.values())
    ...
    def train(self, pt_file_path, num_epochs=20, batch_size=4, learning_rate=1e-4, 
            noise_std=0.01, save_path="enhanced_memory_model", sft_full_texts=None):
        """增强的训练流程 - 支持两组上下文训练"""
        ...
        for epoch in range(num_epochs):
            ...
            if self.is_main_process() and len(dataset) > 0:
                try:
                    ...
                    print(f"\n📋 Epoch {epoch+1} 训练样本示例（索引 {sample_idx}）:")
                    print(f"   上下文（截断的SFT文本）: {context_text[:200]}..." if len(context_text) > 200 else f"   上下文（截断的SFT文本）: {context_text}")
                    print(f"   ─────────────────────────────────────────────────────────────────")
                    print(f"   记忆激活引导: {activation_prompt}")
                    print(f"   回忆结束引导: {end_prompt}")
                    print(f"   ─────────────────────────────────────────────────────────────────")
                    ...
                    if input_tokens is not None and labels is not None:
                        ...
                        if 0 <= recall_label_start < len(labels):
                            recall_label_slice = labels[recall_label_start:recall_label_start + recall_token_count]
                            print(f"   <recall>标签片段: {recall_label_slice}")
                            # 解码显示<recall>标签的实际内容
                            recall_decoded = tokenizer.decode(recall_label_slice, skip_special_tokens=False)
                            print(f"   <recall>标签内容: {recall_decoded}")
