...
try:
    ...
    config_path = os.path.join(script_dir, "server", "config_qwen3vl.yaml")
...
def load_config():
    """加载配置文件"""
    config_path = os.path.join(os.path.dirname(__file__), "server", "config_qwen3vl.yaml")
    with open(config_path, 'r', encoding='utf-8') as f:
...
def load_model_and_processor(config):
    """加载模型和processor"""
    # 查找最新的训练模型
    training_model_dir = config.get("memory", {}).get("training", {}).get("training_model_dir", "server/models/trained")
    if not os.path.isabs(training_model_dir):
        training_model_dir = os.path.join(os.path.dirname(__file__), training_model_dir)
    
    if os.path.exists(training_model_dir):
        models = [d for d in os.listdir(training_model_dir) 
                 if os.path.isdir(os.path.join(training_model_dir, d)) and d.startswith("model_")]
        if models:
            model_path = os.path.join(training_model_dir, sorted(models)[-1])
            _log.info(f"使用训练模型: {model_path}")
        else:
            # 使用token_added模型
            token_added_dir = config.get("memory", {}).get("training", {}).get("token_added_model_dir", "server/models/token_added")
            if not os.path.isabs(token_added_dir):
                token_added_dir = os.path.join(os.path.dirname(__file__), token_added_dir)
            if os.path.exists(token_added_dir):
                models = [d for d in os.listdir(token_added_dir) 
                         if os.path.isdir(os.path.join(token_added_dir, d)) and d.startswith("model_")]
                if models:
                    model_path = os.path.join(token_added_dir, sorted(models)[-1])
    ...
    # 确定正确的device_map（考虑CUDA_VISIBLE_DEVICES的影响）
    cuda_visible = os.environ.get("CUDA_VISIBLE_DEVICES")
    cuda_visible_set = bool(cuda_visible)
    cuda_visible_devices = cuda_visible

    if cuda_visible_set and cuda_visible_devices:
        # 如果设置了CUDA_VISIBLE_DEVICES，使用重新映射后的索引
        device_map_device = "cuda:0"
        _log.info(f"🔧 CUDA_VISIBLE_DEVICES已设置，使用重新映射设备 {device_map_device}")
    else:
        # 直接使用物理设备
        if isinstance(device_config, str) and device_config.startswith("cuda"):
            device_map_device = device_config
        else:
            device_map_device = "cuda:0"
        _log.info(f"🔧 使用物理设备 {device_map_device}")

    model = Qwen3VLForConditionalGeneration.from_pretrained(
        model_path,
        trust_remote_code=True,
        torch_dtype=torch.bfloat16,
        device_map={"": device_map_device}  # 使用正确的设备映射
    )
...
def load_memory_db(config, model, device):
    ...
    memory_db_path = config.get("memory", {}).get("memory_db", {}).get("embeddings_path", "server/models/memory_db/memory_embeddings.pt")
    if not os.path.isabs(memory_db_path):
        memory_db_path = os.path.join(os.path.dirname(__file__), memory_db_path)
...
def test_recall_mechanism_with_custom_generate(model, processor, memory_db, config):
    ...
    # 导入custom_generate函数
    try:
        from api_server_qwen3vl import custom_generate, recall_token_ids
    except ImportError:
        _log.error("❌ 无法导入custom_generate函数，请确保api_server_qwen3vl模块可用")
        return False
    ...
    if recall_token_ids:
        ...
        recall_end_token_id = processor.tokenizer.convert_tokens_to_ids("</recall>")
        memory_pad_token_id = processor.tokenizer.convert_tokens_to_ids("<|memory_pad|>")
    ...
    # 测试程序强制限制为50个token，避免运行时间过长，不使用config中的设置
    max_new_tokens = 500
    ...
    # 设置全局变量（custom_generate函数内部会使用）
    import api_server_qwen3vl as api_module
    api_module.recall_token_ids = {
        "<recall>": recall_token_id,
        "</recall>": recall_end_token_id,
        "<|memory_pad|>": memory_pad_token_id
    }
    api_module.memory_db = memory_db
    ...
    # 调用custom_generate函数（与API服务完全一致的参数）
    try:
        with torch.no_grad():
